...
def prepare_data(df: pd.DataFrame, slow, fast, signal, ema):
    df_an = df.copy()
    df_an = moving_average_convergence_divergence(df_an, n_slow=slow, n_fast=fast, n_signal=signal)
...
def load_data(pair, time_d=1):
    ...
    df = pd.read_pickle(f"./data/forex/{pair}_H{time_d}.pkl")
...
def simulate_params(pair, df, df_m5,  slow, fast, signal, ema, time_d):
    ...
    st.df_results['slow'] = slow
    st.df_results['fast'] = fast
    st.df_results['signal'] = signal
    st.df_results['ema'] = ema
    st.df_results['pair'] = pair

    return st.df_results
...
def get_currency_pairs(currencies: list[str], ic: InstrumentCollection):
    pairs = []
    for p1 in currencies:
        for p2 in currencies:
            pair = f"{p1}_{p2}"
            if pair in ic.instruments.keys():
                pairs.append(pair)
    return pairs

def run_ema_macd(ic: InstrumentCollection):
    pairs = get_currency_pairs(['USD', 'GBP', 'JPY', 'NZD', 'AUD', 'CAD'], ic)
    max_processes = 4
    current = 0
    ...
    while current < len(pairs):
        ...
        if todo < max_processes: max_processes = todo
